...
class RescaleTransform:
    ...
    def __call__(self, images):
        ...
        fromY = self._data_max - self._data_min
        toY = self.max - self.min
        for x in range(len(images)):
            image = images[x]
            toImage = []
            for val in image:
                toImage.append(list(map(lambda x: toY*((x-self._data_min)/fromY)+self.min, val)))
            images[x] = toImage
...
def compute_image_mean_and_std(images):
    ...
    mean = np.mean(images,axis=(0,1,2),keepdims=True)[0][0][0]
    std = np.std(images,axis=(0,1,2),keepdims=True)[0][0][0]
