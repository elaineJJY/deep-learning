...
class Solver(object):
    ...
    def _step(self):
        ...
        model_backward = model.backward(model.forward(X_train))
        loss_grad = loss_func.backward(model.forward(X_train),y_train) / len(X_train)  
        dw =  np.dot(model_backward.T , loss_grad)
        opt.step(dw)
